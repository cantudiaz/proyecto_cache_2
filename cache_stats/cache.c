...
void dump_settings()
{
  /* Modificamos el simulador para que sea más fácil recopilar datos.*/
  ...
  /* La memoria siempre va a ser split*/
  printf("%d,", cache_isize); // tamaño cache I
  printf("%d,", cache_dsize); // tamaño cache D
  printf("%d,", cache_assoc); // tipo asociatividad
  printf("%d,", cache_block_size); // tamaño de bloque
  printf("%s,",
    cache_writeback ? "WRITE BACK" : "WRITE THROUGH"); // write back o through
  printf("%s,",
  	cache_writealloc ? "WRITE ALLOCATE" : "WRITE NO ALLOCATE"); // write allocate o no allocate
...
void print_stats()
{
  ...
  /* Formato más útil para recopilar datos*/
  printf("%d,", cache_stat_inst.accesses); // accesos inst
  printf("%d,", cache_stat_inst.misses); // misses inst
  if (!cache_stat_inst.accesses) // miss rate inst
    printf("FAIL,");
  else
    printf("%2.4f,",
   (float)cache_stat_inst.misses / (float)cache_stat_inst.accesses);
  printf("%d,", cache_stat_inst.replacements); // replacements inst

  // printf(" DATA\n");
  printf("%d,", cache_stat_data.accesses); // accesos data
  printf("%d,", cache_stat_data.misses); // misses data
  if (!cache_stat_data.accesses) // MISS RATE DATA
    printf("FAIL,");
  else
    printf("%2.4f,",
   (float)cache_stat_data.misses / (float)cache_stat_data.accesses);
  printf("%d,", cache_stat_data.replacements); // replacements data

  // printf(" TRAFFIC (in words)\n");
  printf("%d,", cache_stat_inst.demand_fetches +
   cache_stat_data.demand_fetches); // demand fetches
  printf("%d", cache_stat_inst.copies_back +
   cache_stat_data.copies_back); // copies back
}
/************************************************************/
